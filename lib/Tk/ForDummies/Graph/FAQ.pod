...
Tk::ForDummies::Graph is an extension of Canvas widget but it is not possible to scrolled it like this :

  my $LineGraph = $mw->Scrolled('Tk::ForDummies::Graph::Lines')->pack; #wrong

But the idea is to create your graph in a scrolled frame as Pane widget. That is an example.
